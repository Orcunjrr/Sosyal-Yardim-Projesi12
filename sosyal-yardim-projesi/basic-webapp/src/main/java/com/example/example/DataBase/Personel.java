...
public class Personel {
	...
	public static Personel getUserInfoByMail(String userMail) {
		Configuration configuration = new Configuration().configure();
		SessionFactory sessionFactory = configuration.buildSessionFactory();
		try (Session session = sessionFactory.openSession()) {
			String sql = "SELECT * FROM personel WHERE mail = ?";
			NativeQuery<Personel> query = session.createNativeQuery(sql, Personel.class);
			query.setParameter(1, userMail);

			List<Personel> userList = query.getResultList();
			if (!userList.isEmpty()) {
	...
	public static boolean control(String mail, String username) {
			try (Session session = HibernateSessionFactory.getSessionFactory().openSession()) {
				String sql = "SELECT * FROM Personel  WHERE username='" + username + "' OR mail='" + mail + "'";
				NativeQuery<Personel> query = session.createNativeQuery(sql, Personel.class);
				Personel personel = query.uniqueResult();
